<?php
/**
 * API REST pour la gestion des Outputs - TEST
 * Version sécurisée avec validation des entrées
 * Utilise ffp3Outputs2
 */

require_once __DIR__ . '/ffp3-database2.php';
...
/**
 * Log les actions critiques (TEST)
 * 
 * @param string $action Action effectuée
 * @param array $data Données associées
 */
function logAction($action, $data = []) {
    $logFile = __DIR__ . '/actions-test.log';
    $timestamp = date('Y-m-d H:i:s');
    $ip = $_SERVER['REMOTE_ADDR'] ?? 'unknown';
    $logEntry = sprintf(
        "[TEST][%s] IP:%s Action:%s Data:%s\n",
        $timestamp,
        $ip,
        $action,
        json_encode($data)
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if ($action == "outputs_state") {
        // Récupération des états pour l'ESP32 TEST
